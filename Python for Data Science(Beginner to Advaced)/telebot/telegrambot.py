...
def tel_send_message(chat_id,txt):
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            if txt == "/start":
                tel_send_message(chat_id,'Click [here](https://www.google.com) to get connected. Get real time updates on meet and chat requests to maximise your experience.')
                tel_send_message(chat_id,'You are connected to Elitely Bot')
                tel_send_message(chat_id,'You have received a meet up request. Click here to check the details and accept/reject/reschedule')
            # elif txt == "image":
